...
function restoreLiteralIncludes(sourceMd, baked, baseDir) {
  let out = baked;
  for (const directive of sourceMd.match(FENCE_LITERALINCLUDE) || []) {
...
/** Put each `source=` codeExample back: find the baked block with the same
 *  slotId and swap in the source directive. */
function restoreSourceCodeExamples(sourceMd, baked) {
  const directives = findCodeExampleBlocks(sourceMd).filter((b) => b.isSourceDirective);
  let out = baked;
  for (const src of directives) {
    const target = findCodeExampleBlocks(out).find((b) => b.slotId != null && b.slotId === src.slotId);
    if (!target) throw new Error(`source= restore: no exported codeExample with slotId "${src.slotId}"`);
...
export function restoreDirectives(sourceMd, bakedMd, baseDir) {
  return restoreSourceCodeExamples(sourceMd, restoreLiteralIncludes(sourceMd, bakedMd, baseDir));
}
