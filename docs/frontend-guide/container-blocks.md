...
- **\`data-block-selector="+N|-N"\`** — the same directional selector as the carousel `+1` / `-1` below, generalised: **`N` tells how many of the next uids the next page shows** (so `+6` / `-6` for a grid that pages six at a time), which is how the bridge knows the control moves whole pages rather than one slide. Tag a paged container's next / previous controls with it so selecting a child from the sidebar can reach that child even when it is on another page. When the selected child is not in the DOM at all (it lives on a later page), the bridge finds the container, clicks the `+N` (or `-N`) control, waits for the new page to render, and repeats until the child appears — so it also works for a dynamic container (a listing) whose page a given uid lands on isn't knowable ahead of time (the bridge just keeps clicking until the target renders, so `N` need not be exact). For this to work the container must actually **render** its pager: a manual/static container computes its combined paging with the `staticBlocks` helper (which yields `totalPages` / `prev` / `next`) just as a listing does, otherwise there is no control to tag or click. Add `data-linkable-allow` to a control that **navigates itself** — the pager's own click handler changes the page — so `blockClickHandler` skips its normal block-selector handling and a real author click pages normally, while the bridge still synthesises a click to reveal. A carousel whose slides are all in the DOM and which the BRIDGE advances instead uses the `+1` / `-1` step without `data-linkable-allow`.

  > **Render every container's children the same way — always with paging.** A renderer expands a region's children through two helpers that compose: `staticBlocks` windows the static (non-listing) children and `expandListingBlocks` fetches + windows the listing children. Both return `{ items, paging }`, and you thread the running `seen` count from one call into the next (`seen` is *passed between* calls, not shared state) so a container that mixes static cards and a listing pages as ONE combined window. **Paging is not conditional on a listing being present:** a plain grid of slate cards must page (and render a pager) exactly like a grid with a listing. Skipping `staticBlocks` for the "no listing" case — returning all children with no paging — is a bug: the container silently loses its pager, and a child on a later page then has no `data-block-selector="+N"` control for the bridge to reveal it through.
